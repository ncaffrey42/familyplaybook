...
import Screen from '@/pages/auth/LoginScreen';
...
const auth = {
  // Signed OUT: this is the only state in which a user sees the login screen.
  // Handing it a signed-in user sends it into its post-auth redirect and the
  // test hangs — the screen is behaving correctly, the fixture was not.
...
const mount = () => render(
  <MemoryRouter initialEntries={['/login']}><Screen /></MemoryRouter>
);
...
describe('LoginScreen', () => {
  it('renders the sign-in form', async () => {
    mount();
    expect(await screen.findByText(/Welcome to Family Playbook/i)).toBeInTheDocument();
