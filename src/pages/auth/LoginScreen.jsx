...
const LoginScreen = () => {
  ...
  const { signInWithOtp, signInWithGoogle, signInWithDiscord, signInWithFacebook, signInWithApple, signIn, signUp, resetPasswordForEmail, session } = useAuth();
                ...
                {!isForgotPasswordMode && (
                  <>
                    ...
                    {/* Sign in with Apple — required by App Store guideline 4.8
                        alongside other third-party logins. Full-width per
                        Apple's Human Interface Guidelines. */}
                    <Button
                      onClick={handleAppleSignIn}
                      disabled={googleLoading || discordLoading || facebookLoading || appleLoading || loading}
                      className="w-full h-11 rounded-xl bg-black text-white hover:bg-black/90 transition-all shadow-sm hover:shadow-md mb-6 flex items-center justify-center gap-2"
