# Handoff: Family Playbook — mobile app redesign + marketing homepage

> **Updated 20 Aug 2026.** Two changes since the last version: guide category icons are now line
> glyphs instead of solid dots (see *Guide icons*), and the bundle includes the **marketing
> homepage** design (see *Screen 12*).

## Overview

A full-app mobile redesign for **Family Playbook** (`ncaffrey42/familyplaybook`), covering 11 screens
across two modes, plus a 1440px marketing homepage. The app work does two things:
...
4. **Guide rows** — 4 of them, 10px gap. White, 18px radius, 1px `#F0E2D8` border, 15/16px padding.
   Row = 42px circular halo containing a 19px line glyph in the category colour, then title (Nunito 700 16.5px) over meta
...
5. **"BUNDLES"** label + horizontal carousel, 12px gap, bleeding to the screen edges (negative 22px
   margins with matching padding). Cards 158px wide: a 30px solid colour cap holding a 16px white layers glyph (12px left inset), then name + meta.
...
- **Bundles segment** renders full-width cards: 34px colour cap with the white layers glyph (14px inset), then name (Nunito 700 17px) + meta,
...
- **"IN ORDER"** label, then numbered guide rows (`01`, `02`… in Nunito 700 13px `#D8B9C4`, 16px
  wide) with the same halo/glyph/title/meta structure. Order comes from `pack_guides.position`.
...
3. **"<Person> can see"** white card — list with 14px check glyphs (2.6 stroke, cycling raspberry / apricot / mulberry)
...
- "TONIGHT, IN ORDER" + large guide rows: 48px halo, 22px glyph, Fraunces 19px title, 14.5px meta,
...
## 12. Marketing homepage — `/` (web)

A 1440px desktop page, same brand system as the app. Container: `width:1440px; margin:0 auto`,
background cream `#FDF8F3`, body copy `#5E3D4C`, section padding **72px horizontal**, 70–88px
vertical. Build it responsive down to mobile; the design is the desktop end of that range.

Sections, top to bottom:

1. **Header** — logo left, "Log in" + raspberry "Start free" pill right; 22px/72px padding, 1px
   `#F0E2D8` bottom border.
2. **Hero** — two columns. Left: raspberry eyebrow "For the parent who holds it all", Fraunces 600
   **62px/1.05, -.02em** headline, 19px/1.55 body (max-width 520px), raspberry "Build your playbook"
   + blush "See a real one" pills (54px tall), then a 13.5px reassurance line. Right: 24px-radius
   blush panel, 520px tall, with the app home screenshot bleeding off the bottom edge (300px wide,
   top corners 26px).
3. **Pain points** — blush `#F6DFD3` band, 4-column grid (`.95fr 1fr 1fr 1fr`): one photo (300px
   tall) then three short problem statements.
4. **How it works** — "Write a guide / Group it into a bundle / Send one link". Three white cards,
   20px radius, 28px padding, each with a Fraunces number, a 230px blush screenshot well, a Fraunces
   22px title and body copy.
5. **Three kinds of guide** — two columns: copy left, three rows right on cream `#FDF8F3` inside a
   card, 16px radius, each row = 42px halo + 20px category glyph + label + one line of examples.
   This is the same icon system as the app; keep them in sync.
6. **Helper-mode band** — the single mulberry `#5C2A3E` section, 82px padding, with a decorative
   340px `rgba(253,248,243,.06)` circle bleeding off the top-right. Copy left (apricot eyebrow,
   Fraunces 40px cream headline, raspberry CTA), a faked browser window right showing the shared
   helper view.
7. **Pricing** — three cards from the same plan data as the app. See open question 1: the tiers on
   this page must match `plan_entitlements`, not the old WordPress copy.
8. **Testimonial** — blush band, centred, 200px round portrait, Fraunces 500 30px/1.35 quote.
9. **Photo strip** — three-up grid (`1.4fr 1fr 1fr`, 4px gaps, 340px tall).
10. **Closing CTA** — centred, Fraunces 46px, 56px raspberry pill.
11. **Footer**.

**Photography** is the open item: the prototype uses credited Unsplash placeholders (warm, candid,
natural light, no posed stock). Licensed replacements must be dropped in before launch — do not ship
the Unsplash URLs as hotlinks. Direction and credits are recorded in the "Wireframe notes" band at
the bottom of the prototype (that band is annotation, not part of the page).
...
- **The heart-route mark is used inline as UI** (pin toggle, share success, helper header). Its path
  ...
  changing either.
- **Guide icons**: a **line glyph inside a tinted circular halo**. The halo carries the category
  tint, the glyph carries the category colour and meaning. Category → glyph / colour / halo:

  | Category | Glyph | Stroke | Halo |
  |---|---|---|---|
  | How to | Two-row checklist (two ticks + two rules) | `#C25065` | `#F7DEE3` |
  | Find it | Map pin with centre dot | `#F4A259` | `#FDEEE0` |
  | Reference | Open book with centre spine | `#5C2A3E` | `#EEE0E5` |
  | Emergency / urgent | Alert triangle with bang | `#F0705A` | `#FBE0DA` |

  All glyphs are drawn on a 24×24 viewBox, `fill:none`, `stroke-width:2`, round caps and joins.
  Sizes: **19px** in 42px halos (standard rows), **22px** in 48px halos (helper mode), **20px** on the
  marketing site. Bundles use a white **layers** glyph on the colour cap; the Share screen's
  "can see" list uses a **check** glyph (14px, 2.6 stroke) in the cycling accent colour.

  In the codebase these map cleanly onto lucide-react: `ListChecks` (How to), `MapPin` (Find it),
  `BookOpen` (Reference), `AlertTriangle` (Emergency), `Layers` (Bundles), `Check`. Keep
  `GuideIcon.jsx`; restyle it to the halo + `strokeWidth={2}` glyph and repoint `categoryColors` at
  the brand palette. The exact SVG paths used in the prototype are in the logic class of
  `Family Playbook - Mobile.dc.html` (the `GLYPHS` map) if you'd rather match them literally.
...
| File | What it is |
|---|---|
| `Family Playbook - Mobile.dc.html` | The app prototype. Open in a browser. Turn 2 is the on-brand build; turn 1 keeps the navigation rationale. Toggle Parent / Helper mode above the phone and use the screen index to jump. |
| `Family Playbook - Website Homepage.dc.html` | The marketing homepage prototype (1440px). Open in a browser. |
| `image-slot.js` | Runtime the homepage prototype needs for its photo slots. Reference only. |
| `uploads/*.png` | App screenshots used inside the homepage prototype. |
